...
// GET /api/billing → situação da assinatura do usuário logado
billingRouter.get('/billing', requireAuth, (req, res) => {
  res.json(billingStatus(req.user));
});

// POST /api/billing/checkout { name, cpf, phone } → { url } do pagamento
billingRouter.post('/billing/checkout', requireAuth, async (req, res) => {
  ...
  if (s.admin) return res.status(400).json({ error: 'conta de administrador já tem acesso liberado' });
...
// POST /api/billing/sync → confere na AbacatePay se o pagamento do usuário caiu
billingRouter.post('/billing/sync', requireAuth, async (req, res) => {
  try {
    const activated = await syncPayments({ userId: req.user.id });
    res.json({ activated, ...billingStatus(req.user) });
  } catch (err) {
